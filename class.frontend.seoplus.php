...
class frontendSEOPlus
{
	...
	function aheadzen_seo_robots_arg($post_type='')
	{
		$return_arr = array();
		if(get_option("chk_robots_for_".$post_type)=='on')
		{	
			$noindex = get_option("chk_noindex_for_".$post_type);
			$nofollow = get_option("chk_nofollow_for_".$post_type);
			$global = get_option("chk_overide_local_setting_for_".$post_type);
			if($global=='on'){
				$return_arr['noindex'] = 'index'; 
				$return_arr['nofollow'] = 'follow'; 
			}
			if($noindex=='on'){$return_arr['noindex'] = 'noindex';}
			if($nofollow=='on'){$return_arr['nofollow'] = 'nofollow';}
			$return_str = implode(', ',$return_arr);
			if($return_str){
				echo '<!-- SEO+ plugin -->';
				echo '<meta name="robots" content="'.$return_str.'" />'."\n";
			}
		}		
	}
	...
	function add_meta_robots_tag_in_head_section()
	{
		...
		*/
		...
		{
			$this->aheadzen_seo_robots_arg($post_type);
		}elseif($bp->current_component=='activity') //for Activity page
		{
			$this->aheadzen_seo_robots_arg('activity');
			
		}elseif($bp->current_component=='members' && $bp->current_action=='') //for Members listing page
		{
			$this->aheadzen_seo_robots_arg('members');
		}elseif($bp->current_component=='activity' && $bp->current_action=='just-me') //for Members activity page
		{
			$this->aheadzen_seo_robots_arg('members_activity');
		}elseif($bp->current_component=='profile' && $bp->current_action=='public') //for Members profile page
		{
			$this->aheadzen_seo_robots_arg('members_profile');
		}elseif($bp->current_component=='activity' && $bp->current_action=='mentions') //for Members mentions page
		{
			$this->aheadzen_seo_robots_arg('members_mentions');
		}elseif($bp->current_component=='activity' && $bp->current_action=='favorites') //for Members favorites page
		{
			$this->aheadzen_seo_robots_arg('members_favorites');
		}elseif($bp->current_component=='activity' && $bp->current_action=='friends') //for Members friends page
		{
			$this->aheadzen_seo_robots_arg('members_friends');
		}elseif($bp->current_component=='friends' && $bp->current_action=='my-friends') //for Members my-friends page
		{
			//$this->aheadzen_seo_robots_arg('members_friends');
		}elseif($bp->current_component=='friends' && $bp->current_action=='my-friends') //for Members my-friends page
		{
			//$this->aheadzen_seo_robots_arg('members_my-friends');
		}elseif($bp->current_component=='groups' && $bp->current_action=='my-groups') //for Members groups page
		{
			//$this->aheadzen_seo_robots_arg('members_my-groups');
		}elseif($bp->current_component=='groups' && $bp->current_action=='forum') //for Members groups page
		{
			$this->aheadzen_seo_robots_arg('forums');
		}
		elseif($bp->current_component=='groups') //for Groups page
		{
			if($bp->current_action=='home')
			{
				$this->aheadzen_seo_robots_arg('groups');
			}elseif($bp->current_action=='forum')
			{				
			}
		}
		...
		if(trim($post->post_type))
		{
			...
			else
			{
				...
				else if(function_exists('bp_is_user_profile') && bp_is_user_profile())
				{
					global $current_user;
				...
				else if(function_exists('bp_is_user_friends') && bp_is_user_friends())
				{
				...
				else if(function_exists('bp_is_user_groups') && bp_is_user_groups())
				{
				...
				else if(function_exists('bp_is_user_forums') && bp_is_user_forums())
				{
